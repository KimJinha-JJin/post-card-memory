...
/**
 * 사용자가 입력한 문자열을, 각 글자 획을 따라 흰색 외곽선이 둘러싼
 * 다이컷 스티커처럼 그린다. 사각형 흰 배경이 아니라 글자 모양 자체를
 * 따르는 외곽선이어야 하므로, Compose Text 대신 android.graphics.Paint를
 * 흰색 STROKE로 먼저 그리고 그 위에 컬러 FILL로 다시 그리는 표준 기법을
 * 쓴다. PostcardImageExporter의 공유/내보내기 렌더도 동일한 두 겹 그리기
 * 기법을 쓰므로, 화면과 공유 결과의 외곽선이 같은 방식으로 만들어진다.
 *
